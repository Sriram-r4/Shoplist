          ...
          <TouchableOpacity onPress={() => showCardMessage(item)} onLongPress={() => handleItemLongPress(item)}>
            <View style={{ width: wp(30), height: hp(15), alignSelf: 'center' }} key={num + 1} className='bg-teal-200/[0.2] m-1  rounded-2xl'>
              <Card.Content>
                ...
                <Text style={{ height: hp(3.2) }} className= {item.status==="green"?' m-1 text-green-600 text-lg font-medium text-ellipsis'
                :(item.status==="red"?' m-1 text-red-600 text-lg font-medium text-ellipsis'
                :' m-1 text-yellow-600 text-lg font-medium text-ellipsis')}>
                  {item.itemName}
                  </Text>            
