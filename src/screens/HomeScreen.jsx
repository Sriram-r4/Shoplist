...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView style={{
      ...
      <View style={{ width: wp(95), height: hp(22), alignSelf: 'center' }} className='bg-teal-50 m-4 rounded-2xl'>
        ...
        <ScrollView style={{ width: wp(90),height:hp(15), alignSelf: 'center', }} showsVerticalScrollIndicator={false}>
          <FlatList
            data={hitems}
            renderItem={({ item }) => (
              <View style={{ width: wp(50), height: hp(13), alignSelf: 'center' }} key={item.id} className='bg-teal-200/[0.2] m-2  rounded-2xl'>
                <Text style={{ height: hp(4) }} className=' mt-2 mx-2 text-teal-800 text-lg font-medium '>{item.text}</Text>
                <Text style={{ height: hp(7) }} className='m-2 mt-0 truncate  text-teal-800'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras posuere dui ut elementum consectetur. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Mauris ac magna enim. Nulla rutrum, tellus in varius sollicitudin, ipsum turpis auctor ipsum, non lobortis enim lacus eget odio. Pellentesque quis elementum.</Text>
              </View>
            )
            }
            horizontal
      ...
      <View style={{ width: wp(95), height: hp(60), alignSelf: 'center' }} className=' rounded-2xl'>
